...
const DataUtils = {
  ...
  getCycleCountByDay: (processedData) => {
    return Object.keys(processedData)
      .sort()
      .map((day) => {
        const cycles = processedData[day].filter(
          (d) => d.measure === "cycle.durationS"
        );
        return {
  ...
  getEnergyByDay: (processedData) => {
    return Object.keys(processedData)
      .sort()
      .map((day) => {
        const energyMetrics = processedData[day].filter(
          (d) => d.measure === "energy.totalWh"
        );
        const perHour = processedData[day].filter(
          (d) => d.measure === "energy.perHourWh"
        );
        const total = energyMetrics.reduce((sum, m) => sum + m.value, 0);
        const avgPerHour = perHour.length
          ? perHour.reduce((sum, m) => sum + m.value, 0) / perHour.length
          : 0;
        return {
          day: new Date(day).toLocaleDateString("en-US", {
            month: "short",
    ...
    return Object.keys(processedData)
      ...
      .map((day) => {
        ...
        return {
          ...
          phase1: u1.length
            ? u1.reduce((sum, m) => sum + m.value, 0) / u1.length
            : 0,
          phase2: u2.length
            ? u2.reduce((sum, m) => sum + m.value, 0) / u2.length
            : 0,
          phase3: u3.length
            ? u3.reduce((sum, m) => sum + m.value, 0) / u3.length
            : 0,
        };
      });
  },
  ...
  getCurrentByDay: (processedData) => {
    return Object.keys(processedData)
      .sort()
      .map((day) => {
        const inrushMax = processedData[day].filter(
          (d) => d.measure === "inrush.maxPeakA"
        );
        const inrushMean = processedData[day].filter(
          (d) => d.measure === "inrush.meanPeakA"
        );
        const workCurrent = processedData[day].filter(
          (d) => d.measure === "workCurrent.meanAvgA"
        );
        ...
        return {
          ...
          inrushMax: inrushMax.length
            ? inrushMax.reduce((sum, m) => sum + m.value, 0) / inrushMax.length
            : 0,
          inrushMean: inrushMean.length
            ? inrushMean.reduce((sum, m) => sum + m.value, 0) /
              inrushMean.length
            : 0,
          workCurrent: workCurrent.length
            ? workCurrent.reduce((sum, m) => sum + m.value, 0) /
              workCurrent.length
            : 0,
        };
      });
  ...
  getRuntimeByDay: (processedData) => {
    return Object.keys(processedData)
      .sort()
      .map((day) => {
        const durations = processedData[day].filter(
          (d) => d.measure === "cycle.durationS"
        );
        const totalRuntime = durations.reduce((sum, d) => sum + d.value, 0);
        const avgDuration = durations.length
          ? totalRuntime / durations.length
          : 0;
  ...
  calculateUtilization: (processedData) => {
    const allCycles = Object.values(processedData)
      .flat()
      .filter((d) => d.measure === "cycle.durationS");
    ...
    const utilization = Math.min(
      (actualCyclesPerDay / optimalCyclesPerDay) * 100,
      100
    );
  ...
  calculateChamberFullness: (processedData) => {
    ...
    const medium =
      cyclesPerDay < 50 ? 0 : Math.min(40, ((cyclesPerDay - 50) / 100) * 40);
    const high =
      cyclesPerDay < 150
        ? 0
        : Math.min(100, ((cyclesPerDay - 150) / 100) * 100);
  ...
  detectAnomalies: (processedData) => {
    ...
    Object.keys(processedData).forEach((day) => {
      ...
      const errors = data.filter(
        (d) => d.measure === "DI8_Full_Error" && d.value === 1
      );
      if (errors.length > 0) {
        anomalies.push({
          date: day,
  ...
  getAnomalyBreakdown: (processedData) => {
    ...
    anomalies.forEach((a) => {
      const dayKey = new Date(a.date).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
      if (!byDay[dayKey]) {
      ...
      .map((day) => {
        ...
        for (let i = 1; i < kmStates.length; i++) {
          if (kmStates[i].value === 0) {
            const timeDiff =
              (kmStates[i].timestamp - kmStates[i - 1].timestamp) / 1000 / 60;
  ...
  getDigitalInputTimeline: (processedData) => {
    ...
    Object.keys(processedData)
      ...
      .forEach((day) => {
        ...
        const eStops = data.filter(
          (d) => d.measure === "DI2_ES_Overload_Key" && d.value === 1
        );
        const errors = data.filter(
          (d) => d.measure === "DI8_Full_Error" && d.value === 1
        );
        const gates = data.filter(
          (d) => d.measure === "DI5_GateDown" && d.value === 1
        );

        eStops.forEach((e) =>
          events.push({ type: "Overload", time: e.time, status: "error" })
        );
        errors.forEach((e) =>
          events.push({ type: "Full Error", time: e.time, status: "error" })
        );
        gates.forEach((e) =>
          events.push({ type: "Gate Down", time: e.time, status: "success" })
        );
      });
  ...
  calculateMTBF: (processedData) => {
    ...
    const errors = allData.filter(
      (d) => d.measure === "DI8_Full_Error" && d.value === 1
    );
    const cycles = allData.filter((d) => d.measure === "cycle.durationS");

    return errors.length > 0
      ? Math.round(cycles.length / errors.length)
      : cycles.length;
  },
  ...
  calculateMTTR: (processedData) => {
    ...
    for (let i = 1; i < errors.length; i++) {
      if (errors[i - 1].value === 1 && errors[i].value === 0) {
        const repairTime =
          (errors[i].timestamp - errors[i - 1].timestamp) / 1000 / 60;
    ...
    <div className="text-center">
      ...
      <svg width="80" height="120" viewBox="0 0 80 120" className="mx-auto">
        ...
        {mediumPct > 0 && (
          <rect
            x="10"
            y={110 - lowPct * 0.9 - mediumPct * 0.9}
            width="60"
            height={mediumPct * 0.9}
            fill="#ffc107"
          />
        )}
        {highPct > 0 && (
  ...
  return (
    <div>
      ...
      <div className="flex items-center text-xs mb-2">
        <span className="px-2 py-1 bg-green-500 text-white rounded mr-2">
          High (10+)
        </span>
        <span className="px-2 py-1 bg-yellow-400 text-white rounded mr-2">
          Medium (5-10)
        </span>
...
export default function Dashboard() {
  ...
  const handleFileLoad = (content) => {
    ...
    try {
      ...
      const totalCycles = Object.values(byDay)
        .flat()
        .filter((d) => d.measure === "cycle.durationS").length;
  ...
  const cycleCountData = csvData
    ? DataUtils.getCycleCountByDay(processedData)
    : [];
  ...
  const anomalyBreakdown = csvData
    ? DataUtils.getAnomalyBreakdown(processedData)
    : [];
  const recentAnomalies = csvData
    ? DataUtils.detectAnomalies(processedData).slice(-5)
    : [];
  const downtimeData = csvData
    ? DataUtils.calculateDowntime(processedData)
    : [];
  const digitalInputEvents = csvData
    ? DataUtils.getDigitalInputTimeline(processedData)
    : [];
  const heatmapData = csvData
    ? DataUtils.getUtilizationHeatmap(processedData)
    : {};
      ...
      <nav className="bg-white shadow-sm border-b border-gray-200">
        <div className="px-6 py-4 flex items-center justify-between">
          <div className="flex items-center">
            <div className="w-10 h-10 bg-red-600 rounded mr-3 flex items-center justify-center text-white font-bold text-xl">
              K
            </div>
        ...
        <div className="w-56 bg-white min-h-screen shadow-sm">
          <nav className="py-4">
            {sections.map((item) => (
              <div
                key={item.id}
                className="px-6 py-3 flex items-center gap-3 border-l-4 border-transparent"
              >
        ...
        <div className="flex-1 p-6">
          ...
          {csvData && (
            <>
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="flex items-end">
                    <span className="text-xs text-gray-500">
                      Data loaded: {Object.keys(processedData).length} days
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-3 gap-6 mb-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      <h6 className="text-sm font-semibold text-gray-600 mb-4">
                        Utilization Rate
                      </h6>
                  ...
                  <div className="grid grid-cols-2 gap-6 mb-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      <h6 className="text-sm font-semibold text-gray-600 mb-4">
                        Total Runtime Vs Cycle Duration
                      </h6>
              ...
              <CollapsibleSection title="Electrical Health">
                <div className="p-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      <h6 className="text-sm font-semibold text-gray-600 mb-4">
                ...
                <div className="p-6">
                  <div className="grid grid-cols-3 gap-6">
                    <div className="col-span-2 bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      <HeatmapChart
                        title="Utilization Matrix (Hourly × Daily)"
              ...
              {/* ------------------------------------------------------------------------------------------------------------------------------ */}
              <CollapsibleSection title="Energy Management">
                <div className="p-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      <h6 className="text-sm font-semibold text-gray-600 mb-4">
                        Energy Efficiency by Day
                      </h6>
                      <ResponsiveContainer width="100%" height={250}>
              ...
              <CollapsibleSection title="🛡️ Safety & Reliability">
                <div className="p-6">
              ...
              <CollapsibleSection title="📅 EOL Planning">
                <div className="p-6">
                  <div className="grid grid-cols-3 gap-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm text-center">
                      <h6 className="text-sm font-semibold text-gray-600 mb-4">
                        Lifetime Cycles Completed
                      </h6>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm text-center">
                      ...
                      <div className="text-5xl font-bold mb-2">
                        {Math.round(
                          (500000 - stats.totalCycles) /
                            Math.max(stats.avgCycleCount, 1)
                        )}
                      </div>
                      ...
                      <div className="text-left text-xs space-y-1">
                        <div>
                          Current Usage:{" "}
                          <span className="font-semibold">
                            {((stats.totalCycles / 500000) * 100).toFixed(1)}%
                          </span>
                        </div>
                        <div>
                          Target Life:{" "}
                          <span className="font-semibold">500,000 cycles</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                      ...
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart
                          data={cycleCountData.map((d, i) => ({
                            day: d.day,
                            projected:
                              stats.totalCycles + i * stats.avgCycleCount,
                          }))}
                        >
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="day" style={{ fontSize: "10px" }} />
